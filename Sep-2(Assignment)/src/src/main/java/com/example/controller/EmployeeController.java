...
public class EmployeeController {
    ...
    public ResponseEntity<List<Skill>> getSkillsByEmployeeId(@PathVariable Long empId) {
        List<Skill> skills = empServ.getSkillsByEmployeeId(empId);
...
//	}
	...
	EmpOutputDto getEmpByEmail(@PathVariable String email) {
		return empServ.getEmpByEmail(email);
	}
	
	@PostMapping("/employee/update/{empId}/address")
	Employee updateAddress(@PathVariable int empId, @RequestBody Address addr) {
	...
	Employee updateEmpSkills(@PathVariable int empId, @PathVariable int skillId) {
